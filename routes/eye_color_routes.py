...
class EyeColorRouter(BaseRouter):
    ...
    async def get_all_eye_colors(self, db: AsyncSession = Depends(get_db)):
        """Get all eye colors endpoint"""
        logging.info("Getting all eye colors")
        try:
            eye_colors = await eye_color_service.get_all_eye_colors(db)
    ...
    async def get_eye_color_by_id(self, id: int, db: AsyncSession = Depends(get_db)):
        """Get eye color by ID endpoint"""
        logging.info(f"Getting eye color by id: {id}")
        try:
            eye_color = await eye_color_service.get_eye_color_by_id(db, id)
    ...
    async def create_eye_color(self, eye_color: EyeColorCreate, db: AsyncSession = Depends(get_db)):
        """Create eye color endpoint"""
        logging.info(f"Creating eye color: {eye_color.color}")
        try:
            new_eye_color = await eye_color_service.create_eye_color(db, eye_color.model_dump())
    ...
    async def update_eye_color(self, id: int, eye_color: EyeColorCreate, db: AsyncSession = Depends(get_db)):
        """Update eye color endpoint"""
        logging.info(f"Updating eye color with id: {id}")
        try:
            updated_eye_color = await eye_color_service.update_eye_color(db, id, eye_color.model_dump())
    ...
    async def delete_eye_color(self, id: int, db: AsyncSession = Depends(get_db)):
        """Delete eye color endpoint"""
        logging.info(f"Deleting eye color with id: {id}")
        try:
            await eye_color_service.delete_eye_color(db, id)
