...
@router.get("/character/get/{name}", response_model=List[CharacterResponse],
...
def get_character_by_name(name: str, db: Session = Depends(get_db)):
    ...
    if not characters:
        raise HTTPException(status_code=404, detail="No characters found with this name")
    return characters
...
@router.delete("/character/delete/{id}", response_model=CharacterDeleteResponse,
