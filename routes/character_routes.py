...
class CharacterRouter(BaseRouter):
    ...
    async def get_all_characters(self, db: Session = Depends(get_db)):
    ...
    async def get_character_by_name(self, name: str, db: Session = Depends(get_db)):
        """Get characters by name endpoint"""
        logging.info(f"Getting characters by name: {name}")
        try:
            characters = await character_service.get_character_by_name(db, name)
            if not characters:
    ...
    async def create_character(self, character: CharacterCreate, db: Session = Depends(get_db)):
        """Create character endpoint"""
        logging.info(f"Creating character: {character.name}")
        try:
            new_char = await character_service.create_character(db, character.model_dump())
    ...
    async def delete_character(self, id: int, db: Session = Depends(get_db)):
        """Delete character endpoint"""
        logging.info(f"Deleting character with id: {id}")
        try:
            await character_service.delete_character(db, id)
    ...
    async def update_character(self, id: int, character: CharacterCreate, db: Session = Depends(get_db)):
        """Update character endpoint"""
        logging.info(f"Updating character with id: {id}")
        try:
            updated_char = await character_service.update_character(db, id, character.model_dump())
